# Rowan Hennessy's Personal GitHub

## About
I'm currently a first-year Master's student at Columbia University, studying Operations Research with a concentration in Stochastic Modeling. 

Prior to this, I graduated in May, 2025 from Cornell University with a degree in Computer Science and Mathematics

## Coursework

Graduate: 
- Machine Learning for Operations Research and Financial Engineering
- Probability Theory and Statistics
- Financial Engineering
- Optimization Models and Methods
- Programming for Financial Engineering


Undergraduate:
- Computer Science: 
    - Object-Oriented Programming and Data Structures (TA'd)
    - Discrete Structures 
    - Computer System Organization
    - Data Structures and Functional Programming (TA'd)
    - Natural Language Processing
    - Machine Learning
    - Language and Information (TA'd)
    - Analysis of Algorithms
    - Numerical Analysis and Differential Equations
    - Operating Systems
    - Probabilty, Vectors, and Matrices in Computing
- Mathematics: 
    - Calculus I, II, III
    - Statistical Theory and Application
    - Proof Theory
    - Linear Algebra
    - Applicable Algebra
    - Applied Complex Analysis
    - Ordinary Differential Equations
    - Computational Algebra
    - Probability Theory
    - Partial Differential Equations
- Miscellaneous: 
    - Engineering Stochastic Processes
    - Financial Engineering
    - Financial Engineering w/ Stochastic Calculus
